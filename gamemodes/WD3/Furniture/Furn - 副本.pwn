...
ACT::OnFurnitureLoad()
{
	...
	for(new i=0;i<rows;i++)
	{
		if(i<MAX_FURN)
		{
		    ...
		    Furn[i][f_uid]=cache_get_field_content_int(i,"f_uid");
		    ...
		    cache_get_field_content(i,"f_Key",Furn[i][f_Key],mysqlid,129);
		    ...
		    Furn[i][f_sellmoney]=cache_get_field_content_int(i,"f_sellmoney");
...
ACT::AddFurn(models,Float:xx,Float:yy,Float:zz,interiors,worlds,buymoney,furnname[],uidid)
{
    ...
    format(Furn[furnid][f_Key],129,"NULL");
    ...
    Furn[furnid][f_sellmoney]=buymoney;
    Furn[furnid][f_txd]=1;
    Furn[furnid][f_color]=0;
	format(Querys, sizeof(Querys),"INSERT INTO `"SQL_FURN"`(`f_model`,`f_x`,`f_y`,`f_z`,`f_in`,`f_wl`,`f_sellmoney`,`f_name`,`f_uid`)\
	VALUES ('%i','%f','%f','%f','%i','%i','%i','%s','%i')",Furn[furnid][f_model],Furn[furnid][f_x],Furn[furnid][f_y],Furn[furnid][f_z],Furn[furnid][f_in],Furn[furnid][f_wl],Furn[furnid][f_sellmoney],Furn[furnid][f_name],Furn[furnid][f_uid]);
	mysql_query(mysqlid,Querys);
...
ACT::GetClosestFurn(playerid)
{
	new Float:dis, Float:dis2,furnid;
	furnid = -1;
	dis = 99999.99;
	foreach(new c:Furn)
	{
		if(PlayerToPoint(3,playerid,Furn[c][f_x],Furn[c][f_y],Furn[c][f_z],Furn[c][f_in],Furn[c][f_wl]))
		{
			new Float:x1, Float:y1, Float:z1;
			GetPlayerPos(playerid, x1, y1, z1);
			dis2 = floatsqroot(floatpower(floatabs(floatsub(Furn[c][f_x],x1)),2)+floatpower(floatabs(floatsub(Furn[c][f_y],y1)),2)+floatpower(floatabs(floatsub(Furn[c][f_z],z1)),2));
...
public OnPlayerKeyStateChange(playerid, newkeys, oldkeys)
{
	if(Login[playerid])
	{
	    if(PRESSED(KEY_YES))
	    {
	        ...
	        new furnid=GetClosestFurn(playerid);
	        if(furnid!=-1)
			{
			    new caption[100];
			    if(Furn[furnid][f_uid]==-1)
			    {
			        format(caption, sizeof(caption),"系统出售 [%i] %s 售价 $%i ,是否购买?",furnid,Furn[furnid][f_name],Furn[furnid][f_sellmoney]);
			        Dialog_Show(playerid,dl_furn_buy_system,DIALOG_STYLE_MSGBOX,"购买家具",caption, "购买", "返回");
			    }
			    else
			    {
				    if(Furn[furnid][f_uid]==pdate[playerid][uid])
					{
					...
					{
					    if(Furn[furnid][f_sellmoney]>0)
					    {
					        format(caption, sizeof(caption),"%s出售他的家具 [%i] %s 售价 $%i ,是否购买?",GetUidName(Furn[furnid][f_uid]),furnid,Furn[furnid][f_name],Furn[furnid][f_sellmoney]);
			        		Dialog_Show(playerid,dl_furn_buy_player,DIALOG_STYLE_MSGBOX,"购买家具",caption, "购买", "返回");
					    }
					    else
					    {
						    format(caption, sizeof(caption),"[%i] %s 信息",furnid,Furn[furnid][f_name]);
							Dialog_Show(playerid,dl_furn_info,DIALOG_STYLE_MSGBOX,caption,ShowFurnInfo(furnid), "操作", "返回");
						}
					}
				}
		...
		if(PRESSED(KEY_CTRL_BACK))
		{
	        ...
	        if(furnid!=-1)
			{
				if(!strcmp(Furn[furnid][f_Key],"NULL",false))
				{
				}
			}
	    }
...
ShowFurnInfo(furnid)
{
	...
	format(str, sizeof(str),"家具主人:%s\n",GetUidName(Furn[furnid][f_uid]));
	strcat(string,str);
    return string;
}
...
Dialog:dl_furn_buy_system(playerid, response, listitem, inputtext[])
{
	new furnid=PlayerFurn[playerid];
	if(response)
    {
        if(pdate[playerid][cash]<Furn[furnid][f_sellmoney])SendClientMessage(playerid,COLOR_WARNING,"你没有那么多现金");
        else
        {
            GiveCash(playerid,-Furn[furnid][f_sellmoney]);
            Furn[furnid][f_uid]=pdate[playerid][uid];
            Furn[furnid][f_sellmoney]=0;
			format(Querys, sizeof(Querys),"UPDATE `"SQL_FURN"` SET  `f_uid`='%i',`f_sellmoney`='%i' WHERE `"SQL_FURN"`.`f_id` ='%i'",Furn[furnid][f_uid],Furn[furnid][f_sellmoney],Furn[furnid][f_id]);
			mysql_query(mysqlid,Querys,false);
        }
    }
	PlayerFurn[playerid]=-1;
...
Dialog:dl_furn_buy_player(playerid, response, listitem, inputtext[])
{
	new furnid=PlayerFurn[playerid];
	if(response)
    {
		...
		{
            new string[512];
			format(string, sizeof(string),"%s 购买了你的家具,花费 $%i",GetUidName(Furn[furnid][f_uid]),Furn[furnid][f_sellmoney]);
		    if(SystemSendMsgToPlayer(Furn[furnid][f_uid],string,Furn[furnid][f_sellmoney]))
			{
			    GiveCash(playerid,-Furn[furnid][f_sellmoney]);
            	Furn[furnid][f_uid]=pdate[playerid][uid];
            	Furn[furnid][f_sellmoney]=0;
...
/*Dialog:dl_furn_menu_password(playerid, response, listitem, inputtext[])
{
	new furnid=PlayerFurn[playerid];
	if(response)
    {
	    ...
	    if(!strcmp(Furn[furnid][f_Key],hpass,false))
	    {
	        new caption[100];
			format(caption, sizeof(caption),"[%i] %s",furnid,Furn[furnid][f_name]);
			Dialog_Show(playerid,dl_furn_menu,DIALOG_STYLE_TABLIST,caption,ShowFurnMenu(furnid), "操作", "返回");
		...
		{
			SendClientMessage(playerid,COLOR_WARNING,"密码错误,无法操作此家具");
			PlayerFurn[playerid]=-1;
		}
	}
...
ShowFurnMenu(furnid)
{
	...
	strcat(string,str);
	if(!strcmp(Furn[furnid][f_Key],"NULL",false))format(str, sizeof(str),"密码\t无\n");
	else format(str, sizeof(str),"密码\t有\n");
	strcat(string,str);
	...
	strcat(string,str);
	if(Furn[furnid][f_sellmoney]<1)format(str, sizeof(str),"出售设置\t不出售");
	else format(str, sizeof(str),"出售设置\t$%i",Furn[furnid][f_sellmoney]);
	strcat(string,str);
    return string;
}
Dialog:dl_furn_pass(playerid, response, listitem, inputtext[])
{
	if(response)
    {
        new furnid=PlayerFurn[playerid];
        if(strlen(inputtext)<4||strlen(inputtext)>20)return Dialog_Show(playerid,dl_furn_pass,DIALOG_STYLE_INPUT,"家具密码","请设置家具密码,必须大于4个字符,小于20个字符","确定","取消");
        SHA256_PassHash(inputtext,Salt_ACCOUNT,Furn[furnid][f_Key],129);
        format(Querys, sizeof(Querys),"UPDATE `"SQL_FURN"` SET  `f_Key`='%s' WHERE `"SQL_FURN"`.`f_id` ='%i'",Furn[furnid][f_Key],Furn[furnid][f_id]);
		mysql_query(mysqlid,Querys,false);
    }
	PlayerFurn[playerid]=-1;
    return 1;
}
Dialog:dl_furn_sellmoney(playerid, response, listitem, inputtext[])
{
	if(response)
    {
        new furnid=PlayerFurn[playerid];
        if(strval(inputtext)<1)return Dialog_Show(playerid,dl_furn_sellmoney,DIALOG_STYLE_INPUT,"家具价格不能小于 $1","请输入家具的价格","确定","取消");
		Furn[furnid][f_sellmoney]=strval(inputtext);
        format(Querys, sizeof(Querys),"UPDATE `"SQL_FURN"` SET  `f_sellmoney`='%i' WHERE `"SQL_FURN"`.`f_id` ='%i'",Furn[furnid][f_sellmoney],Furn[furnid][f_id]);
		mysql_query(mysqlid,Querys,false);
    }
	PlayerFurn[playerid]=-1;
    return 1;
...
Dialog:dl_furn_menu(playerid, response, listitem, inputtext[])
{
	...
	if(response)
    {
        if(Furn[furnid][f_uid]!=pdate[playerid][uid])return SendClientMessage(playerid,COLOR_WARNING,"这不是你的家具");
		switch(listitem)
        {
           case 0: 
           {
           ...
           case 2: 
           {
                if(!strcmp(Furn[furnid][f_Key],"NULL",false))Dialog_Show(playerid,dl_furn_pass,DIALOG_STYLE_INPUT,"家具密码","请设置家具密码","确定","取消");
                else
                {
              		format(Furn[furnid][f_Key],129,"NULL");
                    format(Querys, sizeof(Querys),"UPDATE `"SQL_FURN"` SET  `f_Key`='%s' WHERE `"SQL_FURN"`.`f_id` ='%i'",Furn[furnid][f_Key],Furn[furnid][f_id]);
           ...
           case 5:ShowFurnTxdBuy(playerid,1);
           case 6:Dialog_Show(playerid,dl_furn_color,DIALOG_STYLE_LIST,"家具颜色",ShowColorStrEx(),"选择","取消");
           case 7:
           {
                if(Furn[furnid][f_sellmoney]>0)
				{
                ...
                else Dialog_Show(playerid,dl_furn_sellmoney,DIALOG_STYLE_INPUT,"家具价格","请输入家具的价格","确定","取消");
           }
        }
    }
    else
